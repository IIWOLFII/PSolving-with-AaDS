...
class LinkedList():
    ...
    def index(self, item): #this should be different for ordered list it should stop like when we search but im lazy oops
        current = self.head
        count = 0
        while current is not None:
            if current.value == item:
...
class OrderedList(LinkedList): #min to max
    def __init__(self):
        super().__init__()
...
class UnOrderedList_O_1(LinkedList):
    def __init__(self):
        super().__init__()
        self.tail = None
